...
def get_ssh_client(host: str, username: str, password: str = None, key_file: str = None, port: int = 22) -> paramiko.SSHClient:
    """
...
def get_directory_inodes(client: paramiko.SSHClient, path: str) -> dict:
    ...
    try:
        ...
        if not output:
            raise RuntimeError(f"No output from remote command or path does not exist: {path}")
...
def main():
    ...
    client = get_ssh_client(args.host, args.username, args.password, args.key, args.port)
